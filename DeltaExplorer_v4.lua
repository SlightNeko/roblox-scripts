...
pcall(function()SG:SetCore("SendNotification",{Title="数据浏览器",Text="加载中",Duration=2})end)
...
local function createRestore()
 ...
 restoreBtn.Position=UDim2.new(0.5,-40,1,-45)
 restoreBtn.BackgroundColor3=Color3.fromRGB(40,40,55)
 restoreBtn.Text="还原"
 restoreBtn.TextColor3=Color3.fromRGB(255,255,255)
 restoreBtn.TextSize=14
 restoreBtn.Font=Enum.Font.SourceSans
 restoreBtn.AutoButtonColor=false
 restoreBtn.Parent=gui.Parent
...
-- Info panel
local ip=tps[1]
L(ip,"玩家: "..lp.Name,Color3.fromRGB(255,200,100));L(ip,"用户ID: "..tostring(lp.UserId),Color3.fromRGB(255,200,100));L(ip,"账号年龄: "..tostring(lp.AccountAge),Color3.fromRGB(255,200,100));L(ip,"场所ID: "..tostring(game.PlaceId),Color3.fromRGB(255,200,100));L(ip,"游戏ID: "..tostring(game.GameId),Color3.fromRGB(255,200,100))
local function roleInfo()
 local char=lp.Character;if char and char:FindFirstChild("Humanoid")then
  local hum=char:FindFirstChild("Humanoid");L(ip,"生命: "..tostring(math.floor(hum.Health)).."/"..tostring(hum.MaxHealth),Color3.fromRGB(100,255,100));L(ip,"速度: "..tostring(hum.WalkSpeed),Color3.fromRGB(100,255,100));L(ip,"跳跃: "..tostring(hum.JumpPower),Color3.fromRGB(100,255,100))
 else L(ip,"无角色",Color3.fromRGB(255,100,100))end
end
...
-- Value Scanner
local vp=tps[3];local vcs={"NumberValue","IntValue","BoolValue","StringValue"};local vcn={"数字值","整数值","布尔值","字符串值"}
...
for i=1,4 do
 local cn=vcs[i]
 local btn=Instance.new("TextButton",vp);btn.Size=UDim2.new(1,-10,0,24);btn.BackgroundColor3=Color3.fromRGB(45,55,70);btn.Text=vcn[i];btn.TextColor3=Color3.fromRGB(200,200,200);btn.TextSize=12;btn.Font=Enum.Font.SourceSans;btn.AutoButtonColor=false;btn.BorderSizePixel=0;Instance.new("UICorner",btn).CornerRadius=UDim.new(0,6)
 btn.MouseButton1Click:Connect(function()doScan(cn)end)
end
-- Live Monitor
local mp=tps[4]
local mh=L(mp,"实例路径:",Color3.fromRGB(255,200,100))
local ib=Instance.new("TextBox",mp);ib.Size=UDim2.new(1,-10,0,24);ib.BackgroundColor3=Color3.fromRGB(30,30,40);ib.TextColor3=Color3.fromRGB(200,200,200);ib.PlaceholderText="实例路径";ib.Text="";ib.Font=Enum.Font.SourceSans;ib.TextSize=13;Instance.new("UICorner",ib).CornerRadius=UDim.new(0,6)
local sb=Instance.new("TextButton",mp);sb.Size=UDim2.new(1,-10,0,24);sb.BackgroundColor3=Color3.fromRGB(45,70,45);sb.Text="开始";sb.TextColor3=Color3.fromRGB(200,200,200);sb.TextSize=13;sb.Font=Enum.Font.SourceSans;sb.AutoButtonColor=false;sb.BorderSizePixel=0;Instance.new("UICorner",sb).CornerRadius=UDim.new(0,6)
local mc=nil
sb.MouseButton1Click:Connect(function()
 if mc then mc:Disconnect();mc=nil;sb.Text="开始";sb.BackgroundColor3=Color3.fromRGB(45,70,45);L(mp,"已停止",Color3.fromRGB(255,200,100));return end
 local p=ib.Text;if p==""then L(mp,"请先输入路径",Color3.fromRGB(255,100,100));return end
 local fn,serr=loadstring("return "..p);if not fn then L(mp,"路径无效: "..tostring(serr),Color3.fromRGB(255,100,100));return end;local ok,tgt=pcall(fn);if not ok or not tgt then L(mp,"路径无效",Color3.fromRGB(255,100,100));return end
 for _,c in ipairs(mp:GetChildren())do if c:IsA("TextLabel")and c~=mh then c:Destroy()end end
 L(mp,"监控中: "..tgt:GetFullName(),Color3.fromRGB(100,255,100));sb.Text="停止";sb.BackgroundColor3=Color3.fromRGB(70,45,45)
...
local sb2=Instance.new("TextBox",sp);sb2.Size=UDim2.new(1,-10,0,24);sb2.BackgroundColor3=Color3.fromRGB(30,30,40);sb2.TextColor3=Color3.fromRGB(200,200,200);sb2.PlaceholderText="搜索内容...";sb2.Text="";sb2.Font=Enum.Font.SourceSans;sb2.TextSize=13;Instance.new("UICorner",sb2).CornerRadius=UDim.new(0,6)
local sbb=Instance.new("TextButton",sp);sbb.Size=UDim2.new(1,-10,0,24);sbb.BackgroundColor3=Color3.fromRGB(45,55,70);sbb.Text="搜索";sbb.TextColor3=Color3.fromRGB(200,200,200);sbb.TextSize=13;sbb.Font=Enum.Font.SourceSans;sbb.AutoButtonColor=false;sbb.BorderSizePixel=0;Instance.new("UICorner",sbb).CornerRadius=UDim.new(0,6)
sbb.MouseButton1Click:Connect(function()
 ...
 for _,obj in ipairs(res)do
  ...
  local tgt=obj;btn.MouseButton1Click:Connect(function()for _,c in ipairs(sp:GetChildren())do if c:IsA("TextButton")and c~=sbb then c.BackgroundColor3=Color3.fromRGB(35,35,50)end end;btn.BackgroundColor3=Color3.fromRGB(70,70,120);pcall(function()SG:SetCore("SendNotification",{Title="已选择",Text=tgt:GetFullName(),Duration=2})end)end)
 end
end)
-- RSPY
local rp=tps[6];local rml=80;local rc=0
local function lr(dir,cn,nm)
 local lb=Instance.new("TextLabel",rp);lb.Size=UDim2.new(1,-10,0,16);lb.BackgroundTransparency=1;lb.Text="["..((dir=="S") and "发送" or (dir=="R") and "接收" or dir).."] "..cn.." -> "..nm;lb.TextXAlignment=Enum.TextXAlignment.Left;lb.Font=Enum.Font.SourceSans;lb.TextSize=11;lb.TextWrapped=true;lb.TextColor3=(dir=="S")and Color3.fromRGB(100,200,255)or Color3.fromRGB(255,150,100)
...
-- Final
pcall(function()SG:SetCore("SendNotification",{Title="数据浏览器",Text="已加载",Duration=2})end)